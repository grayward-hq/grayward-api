...
public abstract class AiReportBase : IChatService
{
        ...
        [EnumeratorCancellation] CancellationToken ct)
    {
        ...
        try
        {
            var request = BuildRequest(systemPrompt, history, userMessage, stream: true);
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                Logger.LogError("[{Provider}] API error {Status}: {Body}",
                    GetType().Name, (int)response.StatusCode, errorBody);
            }
